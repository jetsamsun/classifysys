...
class OtypeController extends  AdminController{
    ...
    public function getVideoOtypeList(Request $request){
        $otypename = $request->input('otypename');
        $dataTmp = VideoOtype::select('oid','otypename','otypename_en','otypename_ks','otype','pic');
        if($otypename){
        ...
        if($dataTmp){
            ...
            foreach($dataTmp as $key=>$value){
                ...
                $dataTmp[$key]['otypename_en'] = !empty($dataTmp[$key]['otypename_en'])?$dataTmp[$key]['html'].$dataTmp[$key]['otypename_en']:'';
                $dataTmp[$key]['otypename_ks'] = !empty($dataTmp[$key]['otypename_en'])?$dataTmp[$key]['html'].$dataTmp[$key]['otypename_ks']:'';
            }
        }
        return response()->json(array('code'=>0,'msg'=>'','data'=>$dataTmp));
    }
    ...
    public function addvideootype(Request $request){
        if($request->isMethod('post')){
            $otypename = $request->input('otypename');
            $otypename_en = $request->input('otypename_en');
            $otypename_ks = $request->input('otypename_ks');
            $otype = $request->input('otype');
            $pic = $request->input('imgval');

            $reg = DB::table('video_otype')->insert(array(
                'otypename'=>$otypename,'otypename_en'=>$otypename_en,'otypename_ks'=>$otypename_ks,'otype'=>$otype,'pic'=>$pic
            ));
    ...
    public function editvideootype(Request $request,$oid){
        ...
        // 分类
        $data = VideoOtype::select('oid','otypename','otypename_en','otypename_ks','otype','pic')
            ->where('oid',$oid)
            ->first()->toArray();
